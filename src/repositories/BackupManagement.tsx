...
const BackupManagement: React.FC = () => {
  ...
  const handleJSONExport = async () => {
    setLoading(true);
    try {
      await importExportService.exportDataJSON();
      addToast('تمت علمية النسخ والحفظ في جهازك (كـ ملف JSON) بنجاح ✅', 'success');
    } catch (e) {
  ...
  const handleJSONImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setLoading(true);
    try {
      const res: any = await importExportService.importDataJSON(e.target.files);
      addToast(`تم الاسترجاع بنجاح! جاري التحديث...`, 'success');
        ...
        <Card className="md:col-span-3 !p-6 bg-gradient-to-br from-slate-900 to-slate-800 text-white border-none shadow-xl relative overflow-hidden">
            <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-6">
              ...
              <div className="flex flex-wrap gap-3 justify-center">
                <Button 
                  variant="primary" 
                  className="h-14 px-8 !rounded-2xl shadow-emerald-500/20 shadow-2xl hover:scale-105 transition-all outline outline-emerald-300"
                  onClick={handleJSONExport}
                  isLoading={loading}
                >
                  <FileJson className="ml-2" size={20} /> تصدير شامل (JSON)
                </Button>
                <label className="cursor-pointer">
                  <input type="file" accept=".json" className="hidden" ref={fileInputRef} onChange={handleJSONImport} />
                  <div className="h-14 px-6 !rounded-2xl bg-amber-600 text-white flex items-center justify-center font-bold hover:bg-amber-500 transition-all shadow-lg mx-2">
                    <Upload className="ml-2" size={20} /> استيراد شامل (JSON)
                  </div>
                </label>
                <div className="h-full w-px bg-slate-600 mx-2 hidden md:block"></div>
                <Button 
                  variant="approve" 
                  ...
                  isLoading={loading}
                >
                  <Database className="ml-2" size={20} /> إنشاء نسخة داخلية
                </Button>
                <Button 
                  variant="secondary" 
                  ...
                  isLoading={loading}
                >
                  <Download className="ml-2" size={20} /> تصدير مشفر .enc
                </Button>
                <label className="cursor-pointer">
                  ...
                  <div className="h-14 px-6 !rounded-2xl bg-blue-600 text-white flex items-center justify-center font-bold hover:bg-blue-500 transition-all shadow-lg">
                    <Upload className="ml-2" size={20} /> استيراد مشفر .enc
                  </div>
                </label>
              </div>
            </div>
