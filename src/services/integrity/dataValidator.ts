...
export const dataValidator = {
  ...
  runDynamicRules: async (entityType: ValidationRule['entityType'], data: any) => {
    ...
    const rules = (await db.getValidationRules()).filter((r: ValidationRule) => r.entityType === entityType && r.isActive);
  ...
  validateSale: async (customerId: string, items: any[], total: number, invoiceId?: string, invoiceDate?: string, isReturn: boolean = false, originalId?: string) => {
    ...
    for (const item of items) {
      ...
      if (!isReturn && availableStock < item.qty) {
        throw new ValidationError(`عجز مخزني في [${item.name}]: المتاح الكلي ${availableStock}، المطلوب ${item.qty}`);
      }
  ...
  validateProduct: async (p: Partial<Product>) => {
    ...
    if (!name || name.length < 2) throw new ValidationError("اسم الصنف قصير جداً.");
    return true;
  }
};
