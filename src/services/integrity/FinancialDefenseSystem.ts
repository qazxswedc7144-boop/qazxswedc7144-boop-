...
export class FinancialDefenseSystem {
  ...
  static startBackgroundScanner() {
    ...
    // Initial run
    this.evaluateSystemThreat().catch(e => console.error("[DefenseSystem] Initial scan failed:", e));

    // Interval: 10 minutes (600,000 ms)
    this.scannerInterval = setInterval(() => {
      console.log("[DefenseSystem] Running 10-minute Integrity Scan...");
      this.evaluateSystemThreat().catch(e => console.error("[DefenseSystem] Background scan failed:", e));
    }, 600000);
  }
}
