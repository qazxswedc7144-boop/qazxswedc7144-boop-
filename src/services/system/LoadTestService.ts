...
import { Sale, Purchase, InventoryTransaction, CashFlow, PerformanceMetric, Product } from '@/types';
...
export class LoadTestService {
  ...
  static async logMetric(operation: string, durationMs: number, metadata?: any) {
    ...
    if (actualTableNames.includes('systemPerformanceLog')) {
      try {
        await db.db.systemPerformanceLog.add(metric);
      } catch (e) {
        console.warn("Failed to write performance metric:", e);
      }
    }
  ...
  static async runBulkSimulation() {
    ...
    if (products.length === 0) {
      ...
      const baseProducts: Product[] = Array.from({ length: 100 }, (_, i) => ({
        ...
        Name: `Test Product ${i}`, // Fallback
        UnitPrice: 15, // Fallback
        StockQuantity: 1000 // Fallback
      }));
