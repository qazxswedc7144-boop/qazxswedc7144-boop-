...
export class SystemTestingEngine {
  ...
  static async validateFIFO(): Promise<TestResult> {
    const layers = await db.db.inventory_layers.toArray();
    const negativeLayers = layers.filter((l: { quantity_remaining?: number }) => (l.quantity_remaining || 0) < 0);
    
    // Check if oldest layers are consumed first
    let orderingError = false;
    const products = [...new Set(layers.map((l: { item_id?: string }) => l.item_id).filter(Boolean))];
    for (const pid of products) {
      const pLayers = layers
        .filter((l: { item_id?: string }) => l.item_id === pid)
        .sort((a: { created_at?: string }, b: { created_at?: string }) => 
          (a.created_at || '').localeCompare(b.created_at || '')
        );
      let foundOpen = false;
      for (const l of pLayers) {
        const isClosed = (l.quantity_remaining || 0) <= 0;
        if (!isClosed) foundOpen = true;
        if (isClosed && foundOpen) {
          orderingError = true;
          break;
        }
      }
