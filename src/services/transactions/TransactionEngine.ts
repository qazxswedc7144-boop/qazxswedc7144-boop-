...
export const processTransaction = async (type: string, data: any) => {
  ...
  for (const item of data.items) {
    ...
    const products = await safeEquals<any>(db.db.products, "name", item.name);
    let product: any = products[0] || null;
  ...
  if (type.includes("sale") && data.customer) {
    const customers = await safeWhereEqual<any>(db.db.customers, "name", data.customer);
  ...
  if (type.includes("purchase") && data.supplier) {
    const suppliers = await safeWhereEqual<any>(db.db.suppliers, "name", data.supplier);
