...
export const SafeModePanel: React.FC = () => {
  ...
  React.useEffect(() => {
    const checkHealth = async () => {
      try {
        ...
        const negStock = products.filter(p => (p.StockQuantity ?? 0) < 0);
        if (negStock.length > 0) errs.push(`وجود مخزون سالب لعدد ${negStock.length} صنف`);
        
        setErrors(errs);
      } catch (e) {
        console.error("SafeMode health check failed:", e);
      }
    };
    checkHealth();
  }, []);
  ...
  const handleReset = async () => {
    try {
      ...
      console.error("handleReset failed:", e);
    }
  };
  ...
  const handleDeactivateSafeMode = async () => {
    try {
      ...
      console.error("handleDeactivateSafeMode failed:", e);
    }
  };
