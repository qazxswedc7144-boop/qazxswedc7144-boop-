...
const CustomerReceiptModule: React.FC<{ onNavigate?: (view: any) => void }> = ({ onNavigate }) => {
  ...
  useEffect(() => {
    if (selectedCustomerId) {
      SalesRepository.getUnpaidByCustomer(selectedCustomerId)
        .then(invoices => {
          setUnpaidSales(invoices);
          setSelectedInvoiceIds(new Set(invoices.map(i => i.id)));
        })
        .catch(err => console.error("Error fetching unpaid sales:", err));
      setAllocations({});
    } else {
      setUnpaidSales([]);
      ...
      <div className="sticky bottom-0 w-full z-50 bg-white border-t border-slate-200 p-3 shadow-lg rounded-b-2xl pb-[calc(14px+env(safe-area-inset-bottom))] px-4">
         <div className="max-w-6xl mx-auto flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 px-2">
            <div className="flex justify-around sm:justify-start gap-6 sm:gap-10">
               <div className="text-center">
                  <p className="text-[10px] font-black text-slate-400 uppercase mb-0.5">إجمالي المقبوض</p>
                  <p className="text-lg font-black text-emerald-600">{totalPaidAmount.toLocaleString()} {currency}</p>
               </div>
               <div className="text-center">
                  <p className="text-[10px] font-black text-slate-400 uppercase mb-0.5">توزيع المبلغ المسدد</p>
                  <p className={`text-lg font-black ${totalAllocated > totalPaidAmount ? 'text-red-500' : 'text-[#1E4D4D]'}`}>{totalAllocated.toLocaleString()} {currency}</p>
               </div>
            </div>
            <Button variant="approve" className="w-full sm:w-auto h-12 px-8 shadow-md text-sm font-bold" onClick={handleConfirmReceipt} isLoading={isProcessing} disabled={totalPaidAmount <= 0 || !selectedCustomerId || totalAllocated > totalPaidAmount} icon={<ArrowDownCircle size={18}/>}>تأكيد وحفظ السند</Button>
         </div>
      </div>
