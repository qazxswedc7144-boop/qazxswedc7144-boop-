...
import { 
  ...
  ArrowUpRight, ArrowDownLeft, Layers
} from 'lucide-react';
...
const AccountingModule: React.FC<AccountingModuleProps> = ({ onNavigate }) => {
      ...
      >
        ...
        >
          ...
          <div className="w-[15%] px-3 text-left">
            <p className={`text-[11px] font-black ${line.runningBalance >= 0 ? 'text-emerald-600' : 'text-red-500'}`}>
  ...
  return (
    <div className="flex flex-col min-h-full h-full bg-[#F8FAFA] font-cairo overflow-hidden" dir="rtl">
      ...
      <header className="shrink-0 bg-white border-b border-slate-100 z-20">
        ...
        <div className="px-6 py-4 flex items-center relative border-b border-slate-50">
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-center">
            ...
            <p className="text-[11px] text-slate-400 font-bold uppercase tracking-[2px] mt-1 opacity-60">GENERAL LEDGER & JOURNAL ENTRIES</p>
          </div>
        </div>
        ...
        <div className="px-6 py-4 flex flex-col md:flex-row gap-4">
          <div className="relative flex-1">
            <input 
              className="w-full h-12 bg-slate-50 border border-slate-100 rounded-[16px] px-8 text-[11px] font-black focus:bg-white focus:border-[#1E4D4D] outline-none shadow-inner transition-all text-right" 
          ...
          <div className="flex items-center gap-2 bg-slate-50 p-1.5 rounded-[16px] border border-slate-100">
            <button 
              ...
              className={`px-6 h-9 rounded-[12px] text-[11px] font-black transition-all flex items-center gap-2 ${showFullArchive ? 'bg-amber-500 text-white shadow-lg' : 'text-slate-400 hover:text-slate-600'}`}
            >
      ...
      <div className="px-0 bg-white border-b border-slate-200 flex items-center text-[11px] font-black text-slate-400 uppercase tracking-widest shrink-0 sticky top-0 z-10">
      ...
      <div className="fixed bottom-10 left-10 z-50">
        ...
        >
          <p className="text-[11px] font-black text-emerald-200/60 uppercase tracking-widest mb-2">إجمالي حجم العمليات</p>
          <p className="text-xl font-black">{stats.totalVolume.toLocaleString()} <span className="text-[11px] opacity-40">{currency}</span></p>
        </motion.div>
      </div>
              ...
              <div className="p-10 space-y-10 overflow-y-auto custom-scrollbar">
                <div className="grid grid-cols-2 gap-8">
                  <div className="bg-slate-50 p-8 rounded-[32px] border border-slate-100 space-y-2">
                    <p className="text-[11px] font-black text-slate-400 uppercase tracking-widest">تاريخ القيد</p>
                    <p className="text-[14px] font-black text-[#1E4D4D]">{new Date(selectedEntry.date).toLocaleDateString('ar-SA')}</p>
                  </div>
                  <div className="bg-slate-50 p-8 rounded-[32px] border border-slate-100 space-y-2">
                    <p className="text-[11px] font-black text-slate-400 uppercase tracking-widest">المرجع / المصدر</p>
                    <p className="text-[14px] font-black text-[#1E4D4D]">{selectedEntry.sourceType} #{selectedEntry.sourceId}</p>
                  </div>
                </div>

                <div className="bg-white p-10 rounded-[40px] border border-slate-100 shadow-sm space-y-4">
                   <p className="text-[11px] font-black text-slate-400 uppercase tracking-widest">البيان المحاسبي</p>
                   <p className="text-[14px] font-black text-[#1E4D4D] leading-relaxed">{selectedEntry.description}</p>
                </div>

                <div className="space-y-6">
                  <h4 className="text-[11px] font-black text-slate-400 uppercase tracking-[3px] mr-4">تفاصيل الحسابات (Double Entry)</h4>
                  <div className="bg-white border border-slate-100 rounded-[40px] overflow-x-auto shadow-sm custom-scrollbar">
                    <table className="w-full text-right min-w-[500px]">
                        ...
                        {selectedEntry.lines.map((l, i) => (
                          <tr key={i} className="hover:bg-slate-50/50 transition-colors">
                            <td className="py-6 px-10">
                              <p className="text-[11px] font-black text-[#1E4D4D]">{l.accountName}</p>
                              <p className="text-[11px] font-bold text-slate-300 uppercase tracking-widest mt-1">ACC: {l.accountId}</p>
                            </td>
                            <td className="py-6 px-10 text-center text-[11px] font-black text-red-500">
                              {l.debit > 0 ? l.debit.toLocaleString() : '-'}
                            </td>
                            <td className="py-6 px-10 text-left text-[11px] font-black text-emerald-600">
                      ...
                      <tfoot className="bg-slate-50/50 border-t border-slate-100">
                        <tr>
                          <td className="py-6 px-10 text-[11px] font-black text-[#1E4D4D]">الإجمالي المتوازن</td>
                          <td className="py-6 px-10 text-center text-[11px] font-black text-red-500">
