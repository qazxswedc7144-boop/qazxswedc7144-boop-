...
const SupplierPaymentModule: React.FC<{ onNavigate?: (view: any) => void }> = ({ onNavigate }) => {
  ...
  useEffect(() => {
    if (selectedSupplierId) {
      PurchaseRepository.getUnpaidBySupplier(selectedSupplierId)
        .then(invoices => {
          setUnpaidInvoices(invoices);
          setSelectedInvoiceIds(new Set(invoices.map(i => i.id)));
        })
        .catch(err => console.error("Error fetching unpaid purchases:", err));
      setAllocations({});
    } else {
      setUnpaidInvoices([]);
      ...
      <div className="fixed bottom-0 left-0 right-0 p-3 bg-white border-t border-slate-200 z-[100] shadow-[0_-4px_16px_rgba(0,0,0,0.06)]">
         <div className="max-w-6xl mx-auto flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 px-2">
            <div className="flex justify-around sm:justify-start gap-6 sm:gap-10">
               <div className="text-center">
                  <p className="text-[10px] font-black text-slate-400 uppercase mb-0.5">إجمالي المسدد</p>
                  <p className="text-lg font-black text-red-500">{totalPaidAmount.toLocaleString()} {currency}</p>
               </div>
               <div className="text-center">
                  <p className="text-[10px] font-black text-slate-400 uppercase mb-0.5">الموزع على الفواتير</p>
                  <p className={`text-lg font-black ${totalAllocated > totalPaidAmount ? 'text-red-500' : 'text-emerald-600'}`}>{totalAllocated.toLocaleString()} {currency}</p>
               </div>
            </div>
            <Button variant="primary" className="w-full sm:w-auto h-12 px-8 shadow-md text-sm font-bold animate-pulse-subtle" onClick={handleConfirmPayment} isLoading={isProcessing} disabled={totalPaidAmount <= 0 || !selectedSupplierId || totalAllocated > totalPaidAmount} icon={<CheckCircle2 size={18}/>}>حفظ وإغلاق السند</Button>
         </div>
      </div>
