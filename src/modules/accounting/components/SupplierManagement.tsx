...
import { 
  ChevronUp, Phone, Plus, 
  MapPin, Calendar, Link as LinkIcon, TrendingUp,
  Users, Truck, Save, Home,
  FileText
} from 'lucide-react';
import { Badge, Button, Modal } from '@/components/shared/SharedUI';
...
const SupplierManagement: React.FC<SupplierManagementProps> = ({ lang, onNavigate }) => {
  ...
  const loadLedger = async (partnerId: string) => {
    try {
      ...
      addToast(isAr ? 'فشل تحميل كشف الحساب ❌' : 'Ledger Load Error ❌', 'error');
    }
  };
      ...
      <header className="p-10 pb-6 shrink-0 bg-white border-b border-slate-100 z-20">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-8 mb-10">
          ...
          <div className="flex items-center gap-4">
            <button 
              ...
              className={`h-11 px-6 text-white rounded-[16px] flex items-center gap-2 text-[11px] font-black shadow-lg transition-all hover:scale-105 ${partnerType === 'supplier' ? 'bg-[#1E4D4D] shadow-emerald-900/20' : 'bg-blue-600 shadow-blue-900/20'}`}
            >
              <Plus size={16} />
            ...
            <button 
              ...
              className="w-11 h-11 bg-slate-50 text-slate-400 rounded-[16px] flex items-center justify-center hover:bg-slate-100 transition-all shadow-sm"
            >
              <Home size={20} />
            </button>
          </div>
        ...
        <div className="flex flex-col md:flex-row gap-6">
          <div className="relative flex-1">
            <input 
              className="w-full h-14 bg-slate-50 border border-slate-100 rounded-[24px] px-8 text-[11px] font-black focus:bg-white focus:border-[#1E4D4D] outline-none shadow-inner transition-all text-right" 
            ...
            return (
              ...
              >
                <div className="p-8 sm:p-10 flex flex-col lg:flex-row lg:items-center justify-between gap-8">
                  ...
                  <div className="flex items-center gap-10 lg:border-r border-slate-100 lg:pr-10 flex-nowrap">
                     <div className="text-center lg:text-right">
                        <p className="text-[11px] font-black text-slate-300 uppercase tracking-widest mb-2">{t.total}</p>
                        <p className={`text-2xl font-black leading-none ${balance >= 0 ? 'text-emerald-600' : 'text-red-500'}`}>
                          {Math.abs(balance).toLocaleString()} <span className="text-[11px] opacity-40 font-bold">{currency}</span>
                        </p>
                     </div>
                  ...
                  {isExpanded && (
                    ...
                    >
                       <div className="p-10 space-y-10">
                          <div className="flex flex-col md:flex-row md:items-center justify-between gap-8 flex-nowrap">
                             <div className="flex items-center gap-4">
                                <TrendingUp size={24} className="text-[#1E4D4D] opacity-30" />
                                <h4 className="text-[11px] font-black text-[#1E4D4D] uppercase tracking-widest">{isAr ? 'كشف الحساب التفصيلي' : 'Detailed Account Statement'}</h4>
                             </div>
                             <div className="flex items-center gap-3 bg-white p-2 rounded-2xl border border-slate-100 shadow-sm flex-nowrap">
                                <div className="flex items-center gap-3 px-4">
                                   <Calendar size={16} className="text-slate-400" />
                                   <input type="date" className="text-[11px] font-black outline-none bg-transparent" value={dateRange.start} onChange={e => setDateRange({...dateRange, start: e.target.value})} />
                                </div>
                                <span className="text-slate-200">|</span>
                                <div className="flex items-center gap-3 px-4">
                                   <input type="date" className="text-[11px] font-black outline-none bg-transparent" value={dateRange.end} onChange={e => setDateRange({...dateRange, end: e.target.value})} />
                                </div>
                             </div>
                          </div>
                          ...
                          <div className="bg-white border border-slate-100 shadow-xl overflow-hidden rounded-[40px]">
                            <div className="overflow-x-auto custom-scrollbar">
                              <table className="w-full text-right">
                                <thead className="bg-slate-50/80 text-[11px] font-black text-slate-400 uppercase tracking-tight border-b border-slate-100">
                                  <tr>
                                ...
                                <tbody className="divide-y divide-slate-50">
                                  {ledgerData.length > 0 ? ledgerData.map((entry, idx) => (
                                    <tr key={entry.id || idx} className="hover:bg-slate-50/50 transition-all group">
                                      <td className="py-6 px-10 text-[11px] font-bold text-slate-400 whitespace-nowrap">{new Date(entry.date).toLocaleDateString(isAr ? 'ar-SA' : 'en-GB')}</td>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={partnerType === 'supplier' ? t.addSupplier : t.addCustomer}>
         <div className="p-8 space-y-8" dir={isAr ? 'rtl' : 'ltr'}>
            <div className="space-y-2">
               <label className="text-[11px] font-black text-slate-400 uppercase tracking-widest mr-2">{isAr ? 'اسم الشريك' : 'Partner Name'}</label>
               <input 
                 className="w-full h-14 bg-slate-50 border border-slate-100 rounded-2xl px-6 text-[11px] font-black focus:bg-white focus:border-emerald-500 outline-none transition-all text-right" 
                 placeholder={isAr ? 'مثال: شركة الأدوية المتحدة' : 'e.g. United Pharma Co.'} 
                 value={newPartner.name} 
                 onChange={e => setNewPartner({...newPartner, name: e.target.value})} 
               />
            </div>
            <div className="grid grid-cols-2 gap-8 flex-nowrap">
               <div className="space-y-2">
                  <label className="text-[11px] font-black text-slate-400 uppercase tracking-widest mr-2">{isAr ? 'رقم الهاتف' : 'Phone Number'}</label>
                  <input 
                    className="w-full h-14 bg-slate-50 border border-slate-100 rounded-2xl px-6 text-[11px] font-black focus:bg-white focus:border-emerald-500 outline-none transition-all text-center" 
                    placeholder="05..." 
               ...
               <div className="space-y-2">
                  <label className="text-[11px] font-black text-slate-400 uppercase tracking-widest mr-2">{isAr ? 'الرصيد الافتتاحي' : 'Opening Balance'}</label>
                  <input 
                    type="number"
                    className="w-full h-14 bg-slate-50 border border-slate-100 rounded-2xl px-6 text-[11px] font-black focus:bg-white focus:border-emerald-500 outline-none transition-all text-center" 
                    placeholder="0.00" 
            ...
            <div className="space-y-2">
               <label className="text-[11px] font-black text-slate-400 uppercase tracking-widest mr-2">{isAr ? 'العنوان' : 'Address'}</label>
               <textarea 
                 className="w-full bg-slate-50 border border-slate-100 rounded-2xl p-6 text-[11px] font-bold h-32 outline-none focus:bg-white focus:border-emerald-500 transition-all text-right" 
