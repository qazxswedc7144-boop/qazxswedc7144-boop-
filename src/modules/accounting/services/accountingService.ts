...
import { CashFlow, AccountingEntry, IntegrityReport, Sale, ReconciliationPoint, InvoiceSettlement, Purchase, UnifiedInvoice } from '@/types';
import { dataValidator } from '@/services/integrity/dataValidator';
import { AccountingRepository } from '@/database/repositories/AccountingRepository';
import { CashFlowRepository } from '@/database/repositories/CashFlowRepository';
...
import { periodService } from '@/services/integrity/periodService';
import { authService } from '@/modules/auth/services/authService';
import { FinancialIntegrityValidator } from '@/services/integrity/FinancialIntegrityValidator';
...
export const accountingService = {
      ...
      customers.forEach(c => {
        ...
        if (buckets.total > 0) {
          result.push({ 
            ...
            buckets,
            oldestInvoiceDate: (oldestDate as Date | null)?.toISOString()
          });
        }
      });
    } else {
      const suppliers = await db.getSuppliers();
      const unpaidPurchases = (await db.getPurchases()).map(mapInvoiceToPurchase).filter(p => p.status === 'UNPAID' && p.invoiceStatus !== 'DRAFT' && p.invoiceStatus !== 'CANCELLED');
  ...
  getEntries: async (): Promise<AccountingEntry[]> => await AccountingRepository.getAllJournalEntries(),
  ...
  recordVoucher: async (type: 'دخل' | 'خرج', name: string, amount: number, category: string, notes: string, partnerId?: string, allocations?: Record<string, number | { amount: number, note?: string }>) => {
    ...
    if (allocations) {
      ...
      for (const id in normalizedAllocations) {
        const allocEntry = normalizedAllocations[id];
        if (allocEntry) {
          simpleAllocations[id] = allocEntry.amount;
        }
      }
    ...
    return await db.runTransaction(async () => {
      ...
      if (allocations && partnerId) {
        for (const invoiceId in normalizedAllocations) {
          const alloc = normalizedAllocations[invoiceId];
          if (alloc && alloc.amount > 0) {
            ...
            await VoucherInvoiceLinkRepository.createLink(voucherId, invoiceId, alloc.amount, alloc.note);
      ...
      return voucherId;
    }, ['vouchers', 'voucherInvoiceLinks', 'journalEntries', 'journalLines', 'accounts', 'suppliers', 'auditLogs', 'settings']);
  },
  ...
  performAutoAdjustment: async (pointId: string, diff: number) => {
    ...
    return await db.runTransaction(async () => {
      ...
      const entry: AccountingEntry = {
        ...
        sourceType: 'ADJUSTMENT', branchId: await db.getCurrentBranchId(),
        lines: [
