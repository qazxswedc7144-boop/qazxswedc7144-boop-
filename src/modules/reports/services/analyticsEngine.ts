...
export const analyticsEngine = {
  ...
  getProductMovementAnalysis: async (days: number = 30): Promise<MovementData[]> => {
    ...
    const recentSales = sales.filter(s => new Date((s as any).Date || s.date || "").getTime() >= cutoffDate.getTime());
    ...
    return products.map(p => {
      ...
      const stockQty = p.StockQuantity ?? (p as any).stock ?? 0;
      const avgInventory = stockQty > 0 ? stockQty : 1; 
