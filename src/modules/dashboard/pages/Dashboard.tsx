...
import { AISummaryPanel } from '@/modules/ai/components/AISummaryPanel';
import { ProfitHealthAnalyzer } from '@/modules/accounting/services/ProfitHealthAnalyzer';
import { AIDashboardEngine } from '@/modules/ai/services/AIDashboardEngine';
import { useLiveMetrics } from '@/modules/dashboard/hooks/useLiveMetrics';
import { motion } from 'motion/react';
import { 
  PackagePlus, FileText, DollarSign, ShieldCheck, RefreshCw, ArrowUpRight, ShoppingCart,
  Clock, ArrowDownCircle, CreditCard, Wallet2, TrendingUp, Activity, BarChart3, PieChart as PieChartIcon,
  Calendar, Package as PackageIcon, History
} from 'lucide-react';
import { 
  ...
  <Card className={`flex flex-col justify-between h-40 border-b-4 ${color} group hover:shadow-2xl transition-all !p-6 bg-white relative overflow-hidden !rounded-[32px]`}>
    ...
    {trend && (
        <div className="flex items-center gap-1 mt-4 relative z-10">
            ...
            <span className="text-[11px] font-black text-emerald-600">{trend}</span>
            <span className="text-[11px] font-black text-slate-300 mr-1">مقارنة بالأمس</span>
        </div>
    )}
...
const Dashboard: React.FC<{ lang?: 'ar', onNavigate?: (view: any, params?: any) => void }> = ({ onNavigate }) => {
  ...
  const { goReports } = useSafeNavigation();
  ...
  useEffect(() => {
    // Sync check functionality removed for offline-only mode
  }, [version]);
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        ...
        const todaySales = transactions.filter(t => t.type === 'sale' && t.date && t.date.startsWith(today));
        ...
        const salesTrend = last7Days.map(date => {
          const daySales = transactions.filter(t => t.type === 'sale' && t.date && t.date.startsWith(date));
          return {
        ...
        if (healthData) {
          console.log("[Dashboard] Loaded latest health snapshot:", healthData.id);
        }
    ...
    fetchStats().catch(err => console.error("[Dashboard] Initial stats fetch failed:", err));
  }, [version]);
  ...
  return (
    <div className="min-h-full h-full flex flex-col bg-[#F8FAFA] font-cairo overflow-hidden w-full relative" dir="rtl">
            ...
            <div className="flex items-center justify-center gap-2 mt-2">
              ...
              {stats.riskScore && (
                <div className={`flex items-center gap-1.5 px-4 py-1.5 rounded-full border shadow-sm ${
                  ...
                  <ShieldCheck size={14} />
                  <span className="text-[11px] font-bold font-cairo">مستوى المخاطرة: {stats.riskScore === 'LOW' ? 'منخفض' : stats.riskScore === 'MEDIUM' ? 'متوسط' : 'مرتفع'}</span>
                </div>
              )}
            </div>
          </motion.div>
      ...
      <main className="flex-1 overflow-y-auto pt-[5px] pb-[5px] px-4 sm:p-10 space-y-[5px] custom-scrollbar">
        
        {/* 1. Main Action Cards - Top Priority */}
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-[5px] sm:gap-4">
          <motion.button 
            whileHover={{ y: -2 }}
          ...
          >
            ...
            <div>
              ...
              <p className="text-[11px] font-bold opacity-80 uppercase tracking-widest">فاتورة بيع جديدة</p>
            </div>
          ...
          >
            ...
            <div>
              ...
              <p className="text-[11px] font-bold opacity-80 uppercase tracking-widest">توريد بضاعة للمخزن</p>
            </div>
            <ArrowUpRight size={18} className="absolute top-4 left-4 opacity-40 group-hover:opacity-100 transition-opacity" />
          </motion.button>
        </div>
        ...
        {/* Quick Access Grid */}
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-[5px] sm:gap-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-[5px] sm:gap-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-[5px] lg:gap-8">
          {/* Side Bento Column */}
          ...
          <Card className="lg:col-span-8 !p-6 !rounded-[32px] bg-white border border-slate-100 shadow-sm">
            <div className="flex items-center justify-between mb-6">
              <div>
                ...
                <p className="text-[11px] font-bold text-slate-400 uppercase tracking-widest">آخر 7 أيام من النشاط المالي</p>
              </div>
              <div className="flex items-center gap-2">
                <div className="px-4 py-2 bg-emerald-50 text-emerald-600 rounded-xl text-[11px] font-black uppercase tracking-widest">مباشر</div>
            ...
            <div className="w-full" style={{ minHeight: '300px' }}>
              {isMounted && (
                <ResponsiveContainer width="100%" height="100%" aspect={2} debounce={50}>
                  ...
                  <YAxis hide />
                  <Tooltip 
                    contentStyle={{ borderRadius: '24px', border: 'none', boxShadow: '0 25px 50px -12px rgb(0 0 0 / 0.15)', fontSize: '11px', fontWeight: '900', padding: '16px' }}
                  />
          ...
          <Card className="lg:col-span-4 !p-6 !rounded-[32px] bg-white border border-slate-100 shadow-sm flex flex-col">
            <div className="flex items-center justify-between mb-6">
              <div>
                ...
                <p className="text-[11px] font-bold text-slate-400 uppercase tracking-widest">آخر الحركات</p>
              </div>
              <button onClick={() => onNavigate?.('logs')} className="text-[11px] font-black text-emerald-600 uppercase tracking-widest hover:underline">عرض الكل</button>
            </div>
            <div className="flex-1 space-y-5">
              {stats.recentTransactions.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center opacity-20 italic font-bold text-[11px] space-y-4">
                  <Activity size={48} />
                ...
                stats.recentTransactions.map((tx, idx) => (
                  ...
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className="text-[11px] font-black text-[#1E4D4D]">{tx.type === 'sale' ? 'مبيعات' : 'مشتريات'}</p>
                        <p className="text-[11px] font-bold text-slate-400">{new Date(tx.date).toLocaleTimeString('ar-SA', { hour: '2-digit', minute: '2-digit' })}</p>
                      </div>
                    </div>
                    <div className="text-left">
                      <p className={`text-[11px] font-black ${tx.type === 'sale' ? 'text-emerald-600' : 'text-blue-600'}`}>{(tx.amount ?? 0).toLocaleString()} {currency}</p>
                    </div>
                  </motion.div>
                ))
              )}
...
const QuickActionBtn = ({ icon, label, onClick, color }: { icon: React.ReactNode, label: string, onClick: () => void, color: string }) => (
  ...
  >
    ...
    <span className="text-[11px] font-black text-[#1E4D4D] uppercase tracking-widest text-center">{label}</span>
  </motion.button>
);
