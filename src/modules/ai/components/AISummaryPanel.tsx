...
import { BehaviorMonitor } from '@/services/audit/BehaviorMonitor';
import { AIDashboardEngine } from '../services/AIDashboardEngine';
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.error("AISummary fetch failed:", e);
      }
    };
    fetchData();
  }, []);
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card className="flex flex-col justify-between h-40 border-b-4 border-red-500 group hover:shadow-2xl transition-all !p-6 bg-white relative overflow-hidden !rounded-[32px]">
          <div className="flex justify-between items-start relative z-10">
            <div className="space-y-1">
              <p className="text-[9px] font-black text-slate-400 uppercase tracking-[2px] opacity-80">تنبيهات نشطة</p>
              <h2 className="text-2xl font-black text-[#1E4D4D] leading-none">{alerts.length} <span className="text-[9px] opacity-30 font-black uppercase">تنبيه</span></h2>
            </div>
          ...
          <div className="flex items-center gap-1 mt-4 relative z-10">
            ...
            <span className="text-[9px] font-black text-red-600">منها {alerts.filter(a => a.severity === 'CRITICAL' || a.severity === 'HIGH').length} عالية</span>
          </div>
        ...
        <Card className="flex flex-col justify-between h-40 border-b-4 border-emerald-500 group hover:shadow-2xl transition-all !p-6 bg-white relative overflow-hidden !rounded-[32px]">
          <div className="flex justify-between items-start relative z-10">
            <div className="space-y-1">
              <p className="text-[9px] font-black text-slate-400 uppercase tracking-[2px] opacity-80">حالة الربحية</p>
          ...
          <div className="flex items-center gap-1 mt-4 relative z-10">
            ...
            <span className="text-[9px] font-black text-emerald-600">هامش الربح: {health?.grossProfitPercent?.toFixed(1)}%</span>
          </div>
          <div className="absolute -right-4 -bottom-4 w-24 h-24 bg-slate-50 rounded-full opacity-50 group-hover:scale-150 transition-transform duration-700"></div>
        </Card>
        ...
        <Card className="flex flex-col justify-between h-40 border-b-4 border-purple-500 group hover:shadow-2xl transition-all !p-6 bg-white relative overflow-hidden !rounded-[32px]">
          <div className="flex justify-between items-start relative z-10">
            <div className="space-y-1">
              <p className="text-[9px] font-black text-slate-400 uppercase tracking-[2px] opacity-80">المستخدم الأكثر نشاطاً</p>
          ...
          <div className="flex items-center gap-1 mt-4 relative z-10">
            ...
            <span className="text-[9px] font-black text-purple-600">درجة المخاطرة: {riskyUsers[0]?.riskScore?.toFixed(0)}</span>
          </div>
          <div className="absolute -right-4 -bottom-4 w-24 h-24 bg-slate-50 rounded-full opacity-50 group-hover:scale-150 transition-transform duration-700"></div>
        </Card>
        ...
        <Card className="flex flex-col justify-between h-40 border-b-4 border-blue-500 group hover:shadow-2xl transition-all !p-6 bg-white relative overflow-hidden !rounded-[32px]">
          <div className="flex justify-between items-start relative z-10">
            <div className="space-y-1">
              <p className="text-[9px] font-black text-slate-400 uppercase tracking-[2px] opacity-80">الفاتورة الأكثر تعديلاً</p>
          ...
          <div className="flex items-center gap-1 mt-4 relative z-10">
            ...
            <span className="text-[9px] font-black text-blue-600">عدد التعديلات: {mostEdited?.versionNumber || 0}</span>
          </div>
          <div className="absolute -right-4 -bottom-4 w-24 h-24 bg-slate-50 rounded-full opacity-50 group-hover:scale-150 transition-transform duration-700"></div>
        </Card>
      </div>
      ...
      {recommendations && (
        <Card className="!p-6 !rounded-[24px] bg-white border border-slate-100 shadow-sm">
          ...
          <div className="text-[9px] font-black text-slate-600 leading-relaxed whitespace-pre-wrap">
            {recommendations}
          </div>
        </Card>
      )}
