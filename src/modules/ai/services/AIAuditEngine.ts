...
import { Sale, Purchase, InvoiceItem, UnifiedInvoice } from '@/types';
...
export class AIAuditEngine {
  ...
  static async auditInvoice(type: 'SALE' | 'PURCHASE', invoice: Sale | Purchase, _items: InvoiceItem[], userId: string) {
    ...
    // Check for rapid repost cycle (repeated edits)
  ...
  private static async getEntityAvgAmount(entityId: string, type: 'SALE' | 'PURCHASE'): Promise<number> {
    ...
    if (type === 'SALE') {
      const sales = await safeWhereEqual<any>(db.sales, 'customerId', entityId);
      if (sales.length === 0) return 0;
      return sales.reduce((sum, s) => sum + (s.finalTotal || 0), 0) / sales.length;
    } else {
      const purchases = await safeWhereEqual<any>(db.purchases, 'partnerId', entityId);
      if (purchases.length === 0) return 0;
      return purchases.reduce((sum, p) => sum + (p.totalAmount || 0), 0) / purchases.length;
    }
  }
  ...
  private static async checkDuplicateAmount(amount: number, type: 'SALE' | 'PURCHASE', currentId: string): Promise<boolean> {
    ...
    if (type === 'SALE') {
      ...
      if (!db.purchases) return false;
      const duplicates = await db.purchases
        .where('date').above(oneHourAgo)
  ...
  static async runDailyScan() {
    ...
    for (const sale of sales) {
      await this.auditInvoice('SALE', sale, sale.items, sale.Created_By || 'system');
    }
    for (const purchase of purchases) {
      await this.auditInvoice('PURCHASE', purchase, purchase.items, purchase.Created_By || 'system');
    }
  ...
  static async storeMonthlyMetrics() {
    ...
    for (const m of metrics) {
      await db.historicalMetrics.put(m);
    }
  ...
  static async getHistoricalMetric(type: 'AVG_SALE' | 'AVG_PURCHASE' | 'AVG_MARGIN' | 'AVG_EDITS'): Promise<number> {
    const metrics = await safeWhereEqual<any>(db.historicalMetrics, 'type', type);
