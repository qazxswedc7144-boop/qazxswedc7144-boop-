...
export const BackupService = {
  ...
  /**
   * 6. GOOGLE APPS SCRIPT BACKUP UPLOAD
   */
  async uploadBackup(password: string): Promise<void> {
    if (!BACKUP_URL || BACKUP_URL.includes("PUT_YOUR_APPS_SCRIPT_URL_HERE")) {
      console.warn("Backup sync skipped: Google Apps Script URL not configured.");
      throw new Error("لم يتم إعداد رابط المزامنة السحابية. يرجى إعداده في الإعدادات أولاً.");
    }
    ...
    try {
      ...
      const payload = {
        secret: "12345", // Optional security password
        device_id: data.device_id,
        timestamp: data.timestamp,
        backup_data: encrypted
      };

      // Using fetch with a timeout to prevent long hangs
      ...
      try {
        ...
        if (!response.ok && response.status !== 0) { // status 0 is for opaque responses if we used no-cors
          throw new Error(`Server returned ${response.status}: ${response.statusText}`);
        }

        console.log("Backup uploaded to Google Apps Script successfully.");
      } catch (fetchErr: any) {
        // If it's a CORS issue, the fetch might fail but the request might have actually reached the server
        // Apps Script web apps are tricky with CORS.
        console.warn("Fetch encountered an issue, but backup might have been sent:", fetchErr);
        if (fetchErr.message?.includes('Failed to fetch')) {
          // This is often a CORS issue even if the request succeeded.
          // We'll log it as a warning instead of a hard error if we suspect it's just CORS.
          console.info("Possible CORS restriction, but request likely sent to Apps Script.");
        } else {
          throw fetchErr;
        }
      } finally {
      ...
      if (e.name === 'AbortError') {
        throw new Error("انتهت مهلة الاتصال بالخادم السحابي. يرجى التحقق من اتصال الإنترنت.");
      }
      console.error("Backup upload failed", e);
      throw new Error(`فشل رفع النسخة الاحتياطية: ${e.message || 'خطأ في الاتصال'}`);
    }
  },
