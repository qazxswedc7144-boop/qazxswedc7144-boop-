...
const StatMiniCard = React.memo(({ label, value, icon, color, unit, trend }: { label: string, value: string | number, icon: React.ReactNode, color: string, unit?: string, trend?: string }) => (
  <Card className={`flex flex-col justify-between h-40 border-b-4 ${color} group hover:shadow-2xl transition-all !p-6 bg-white relative overflow-hidden !rounded-[32px]`}>
    <div className="flex justify-between items-start relative z-10">
        ...
        <div className={`w-12 h-12 bg-slate-50 text-[#1E4D4D] rounded-2xl flex items-center justify-center text-xl shadow-inner group-hover:scale-110 transition-transform`}>{icon}</div>
    </div>
    {trend && (
    ...
    <div className="h-full flex flex-col bg-[#F8FAFA] font-['Cairo'] overflow-hidden" dir="rtl">
      ...
      <main className="flex-1 overflow-y-auto p-6 sm:p-10 space-y-12 custom-scrollbar">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-8 grid grid-cols-1 sm:grid-cols-2 gap-6">
            <motion.button 
              ...
              className="group h-64 bg-gradient-to-br from-[#10B981] to-[#059669] text-white p-10 rounded-[48px] flex flex-col justify-between text-right transition-all shadow-2xl shadow-emerald-900/20 relative overflow-hidden"
            >
              <div className="relative z-10">
                <div className="w-16 h-16 bg-white/20 rounded-3xl flex items-center justify-center mb-6 backdrop-blur-md shadow-xl">
                  <ShoppingCart size={32} />
                </div>
                <h3 className="text-3xl font-black tracking-tight mb-2">المبيعات</h3>
                <p className="text-xs font-bold opacity-80 uppercase tracking-widest">فاتورة بيع جديدة</p>
              </div>
              ...
              <ArrowUpRight size={24} className="absolute top-10 left-10 opacity-40 group-hover:opacity-100 transition-opacity" />
            </motion.button>

            <motion.button 
              whileHover={{ y: -5 }}
              onClick={() => onNavigate?.('purchases')}
              className="group h-64 bg-gradient-to-br from-[#1E4D4D] to-[#0f2a2a] text-white p-10 rounded-[48px] flex flex-col justify-between text-right transition-all shadow-2xl shadow-emerald-900/20 relative overflow-hidden"
            >
              <div className="relative z-10">
                <div className="w-16 h-16 bg-white/20 rounded-3xl flex items-center justify-center mb-6 backdrop-blur-md shadow-xl">
                  <PackagePlus size={32} />
                </div>
                <h3 className="text-3xl font-black tracking-tight mb-2">المشتريات</h3>
                <p className="text-xs font-bold opacity-80 uppercase tracking-widest">توريد بضاعة للمخزن</p>
              </div>
              <div className="absolute -right-10 -bottom-10 w-48 h-48 bg-white/10 rounded-full blur-3xl group-hover:scale-150 transition-transform duration-700"></div>
              <ArrowUpRight size={24} className="absolute top-10 left-10 opacity-40 group-hover:opacity-100 transition-opacity" />
            </motion.button>

            {/* AI Summary Panel - Integrated into Bento */}
          ...
          <div className="lg:col-span-4 space-y-6">
            <Card className="!p-8 !rounded-[48px] bg-white border border-slate-100 shadow-sm h-full flex flex-col justify-between">
              <div className="flex items-center justify-between mb-8">
                <div>
                  <h3 className="text-lg font-black text-[#1E4D4D]">توزيع الأصناف</h3>
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">حسب التصنيف الرئيسي</p>
                </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          <StatMiniCard label="إيرادات اليوم" value={stats.todaySalesTotal} icon={<DollarSign size={24} />} color="border-emerald-500" unit={currency} trend="+12%" />
          ...
          <StatMiniCard label="مبيعات الشهر" value={stats.monthSalesTotal} icon={<Activity size={24} />} color="border-purple-500" unit={currency} trend="+5%" />
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          {/* Sales Trend */}
          <Card className="lg:col-span-8 !p-10 !rounded-[56px] bg-white border border-slate-100 shadow-sm">
            <div className="flex items-center justify-between mb-10">
              <div>
                <h3 className="text-xl font-black text-[#1E4D4D]">اتجاه المبيعات الأسبوعي</h3>
                <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">آخر 7 أيام من النشاط المالي</p>
              </div>
          ...
          {/* Recent Transactions */}
          <Card className="lg:col-span-4 !p-10 !rounded-[56px] bg-white border border-slate-100 shadow-sm flex flex-col">
            <div className="flex items-center justify-between mb-10">
              <div>
                <h3 className="text-xl font-black text-[#1E4D4D]">أحدث العمليات</h3>
                <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">آخر الحركات</p>
              </div>
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-lg ${tx.type === 'sale' ? 'bg-emerald-100 text-emerald-600' : 'bg-blue-100 text-blue-600'}`}>
                        {tx.type === 'sale' ? <ShoppingCart size={20} /> : <PackagePlus size={20} />}
                      </div>
                      <div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-6">
          <QuickActionBtn icon={<Users size={20} />} label="العملاء" onClick={() => onNavigate?.('partners')} color="bg-blue-500" />
          <QuickActionBtn icon={<PackageIcon size={20} />} label="المخزون" onClick={() => onNavigate?.('inventory')} color="bg-purple-500" />
          <QuickActionBtn icon={<Wallet2 size={20} />} label="المحاسبة" onClick={() => onNavigate?.('accounting')} color="bg-amber-500" />
          <QuickActionBtn icon={<History size={20} />} label="الأرشيف" onClick={() => onNavigate?.('invoices-archive')} color="bg-slate-500" />
          <QuickActionBtn icon={<ShieldCheck size={20} />} label="التدقيق" onClick={() => onNavigate?.('audit-history')} color="bg-emerald-600" />
          <QuickActionBtn icon={<Clock size={20} />} label="تعمير الذمم" onClick={() => onNavigate?.('aging-report')} color="bg-red-500" />
          <QuickActionBtn icon={<Settings size={20} />} label="الإعدادات" onClick={() => onNavigate?.('settings')} color="bg-indigo-500" />
        </div>
...
const QuickActionBtn = ({ icon, label, onClick, color }: { icon: React.ReactNode, label: string, onClick: () => void, color: string }) => (
  <motion.button 
    ...
    whileTap={{ scale: 0.95 }}
    onClick={onClick}
    className="flex flex-col items-center gap-4 p-6 bg-white border border-slate-100 rounded-[32px] shadow-sm hover:shadow-xl transition-all group"
  >
    ...
    <span className="text-[11px] font-black text-[#1E4D4D] uppercase tracking-widest">{label}</span>
  </motion.button>
);
